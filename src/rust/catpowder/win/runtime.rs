...
impl SharedCatpowderRuntime {
    ...
    fn reprogram(
        ...
        trace!(
            "reprogram(): new_rules: {:?}; roll_back: {}, stop_idx: {}",
            new_rules.unwrap_or(&inner_self.rules),
            roll_back,
            stop_idx
        );
        ...
        for (idx, rx) in inner_self.rx_rings.iter_mut().take(stop_idx).enumerate() {
            if let Err(e) = rx.reprogram(&mut inner_self.api, &new_rules.unwrap_or(&inner_self.rules)) {
                error!("Failed to reprogram rule set for queue {}: {:?}", idx, e);
                if roll_back {
                    error!("Rolling back to previous XDP rule set");
    ...
    fn add_rule(&mut self, protocol: Protocol, port: u16) -> Result<(), Fail> {
        trace!("add_rule(): protocol={:?}, port={}", protocol, port);
    ...
    pub fn remove_rule(&mut self, protocol: Protocol, port: u16) -> Result<(), Fail> {
        trace!("remove_rule(): protocol={:?}, port={}", protocol, port);
